...
namespace MainProject.UI.DocumentPages
{
    ...
    public partial class pp_GenerateAssemblies : Page
    {
        ...
        private async Task<object> GenerateAllAssemblies(IProgress<string> progress, CancellationToken cancelToken)
        {
            ...
            foreach (var cultureItem in Cultures)
            {
                ...
                foreach (var assemblyItem in Assemblies)
                {
                    ...
                    string satelliteTargetFileName = System.IO.Path.GetFileName(assemblyItem.Assembly.Assembly.DefaultResourceFile);
                    AssemblyGenerationResult result = new AssemblyGenerationResult(satelliteTargetFileName, cultureItem.Culture.CultureCode);
                    ...
                    try
                    {
                        ...
                        string satelliteTargetFullPath = System.IO.Path.Combine(outputDirectory, satelliteTargetFileName);

                        progress.Report(StringUtils.String("GenerationAssemblyForCulture", satelliteTargetFileName, cultureItem.Culture.CultureCode));
                        ...
                        await Task.Run(() =>
                        {
                            ...
                            loadedAssembly.GenerateSatelliteAssembly(cultureItem.Culture.CultureCode, translatedStrings, satelliteTargetFullPath);
                            if (satelliteTargetFullPath.Contains("Build_App"))
                            {
                                sourceFullPath = satelliteTargetFullPath.Replace("Build_App", "Source_App").Replace("IHFood.PigSpineCutMeasurer_x86_Debug", System.IO.Path.Combine("IHFood.PigSpineCutMeasurer", "Localization"));
                                loadedAssembly.GenerateSatelliteAssembly(cultureItem.Culture.CultureCode, translatedStrings, sourceFullPath);
                            }
                        });
        ...
        private string SatelliteFileName(TranslatedAssembly assembly, TranslationCulture culture)
        {
